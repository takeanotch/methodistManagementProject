...
// export default function ChefDepartementConference() {
...
interface ChefDistrict {
  ...
  fidele_nom: string
  fidele_prenom: string
  fidele_contact?: string
  date_nomination: string
  est_actif: boolean
   role_id?: number        // ✅ Ajouté
  role_nom?: string       // ✅ Ajouté
  role_label?: string     // ✅ Ajouté
  role_couleur?: string  
}
...
export default function ChefDepartementConference() {
  ...
  const [chefsDistrict, setChefsDistrict] = useState<ChefDistrict[]>([])
  ...
  // État pour l'accordéon des stats
...
{/* Dans la section d'affichage des chefs */}
const getRoleLabel = (chef: ChefDistrict) => {
  if (chef.role_label) return chef.role_label
  if (chef.role_nom) {
    ...
    return roleMapping[chef.role_nom] || chef.role_nom
  }
  if (chef.role_id) return `Rôle #${chef.role_id}`
  return 'Rôle non défini'
}

const getRoleBadgeColor = (chef: ChefDistrict) => {
  const nomRole = chef.role_nom || ''
  ...
  const loadData = async () => {
    try {
      ...
      // Charger les stats et les chefs de district
      await loadStatsAndChefs(districtsData, info.departement_id)
...
//   const loadStatsAndChefs = async (districtsData: District[], departementId: number) => {
//     setStatsLoading(true)
//     try {
//       const { supabase } = await import('@/lib/supabase')
      
//       // 1. Récupérer toutes les paroisses des districts de la conférence
//       const { data: paroisses, error: paroissesError } = await supabase
//         .from('paroisse')
//         .select('id, nom, district_id')
//         .in('district_id', districtsData.map(d => d.id))
...
//       // 2. Récupérer tous les fidèles du département (actifs)
...
//       if (fidelesError) {
//         console.error('Erreur fidèles:', fidelesError)
//         return
//       }

//       // 3. Calculer les stats par district et paroisse
//       const paroissesMap = new Map<number, Paroisse>()
//       paroisses?.forEach(p => {
//         paroissesMap.set(p.id, { id: p.id, nom: p.nom, district_id: p.district_id })
//       })

//       const stats: DistrictStats[] = districtsData.map(district => {
//         const districtParoisses = paroisses?.filter(p => p.district_id === district.id) || []
//         const paroisseIds = new Set(districtParoisses.map(p => p.id))
        
//         const fidelesDuDistrict = fideles?.filter(f => paroisseIds.has(f.paroisse_id)) || []
        
//         const paroissesStats: ParoisseStats[] = districtParoisses.map(paroisse => {
//           const fidelesParoisse = fidelesDuDistrict.filter(f => f.paroisse_id === paroisse.id)
//           return {
//             paroisse_id: paroisse.id,
//             paroisse_nom: paroisse.nom,
//             total_fideles: fidelesParoisse.length
//           }
//         }).filter(p => p.total_fideles > 0)
//         .sort((a, b) => b.total_fideles - a.total_fideles)

//         return {
//           district_id: district.id,
//           district_nom: district.nom,
//           total_fideles: fidelesDuDistrict.length,
//           paroisses: paroissesStats
//         }
//       })

//       setDistrictStats(stats.filter(s => s.total_fideles > 0))

//       // 4. Récupérer les chefs de district pour ce département
...
//       if (chefsError) {
//         console.error('Erreur chefs:', chefsError)
//         return
//       }

//       const chefsFormatted: ChefDistrict[] = (chefs || []).map((chef: any) => {
...
//           fidele_contact: fidele?.contact || '',
//           date_nomination: chef.date_nomination,
//           est_actif: chef.est_actif
//         }
//       })

//       setChefsDistrict(chefsFormatted)

//     } catch (error) {
//       console.error('Erreur loadStatsAndChefs:', error)
//     } finally {
...
const loadStatsAndChefs = async (districtsData: District[], departementId: number) => {
  ...
  try {
    ...
    const { data: fideles, error: fidelesError } = await supabase
      .from('fidele_departement')
      .select('id, paroisse_id, fidele_id')
      .eq('departement_id', departementId)
      .eq('est_actif', true)

    if (fidelesError) {
      console.error('Erreur fideles:', fidelesError)
      setStatsLoading(false)
      return
    }

    // 3. Calculer les stats par district et paroisse
    const paroissesMap = new Map<number, { id: number; nom: string; district_id: number }>()
    paroisses?.forEach(p => {
      paroissesMap.set(p.id, { id: p.id, nom: p.nom, district_id: p.district_id })
    })
    ...
    // 4. Récupérer les chefs de district pour ce département
    // APPROCHE INSPIRÉE DU COMPOSANT ADMIN : jointure avec la table role
    const { data: chefs, error: chefsError } = await supabase
      .from('chef_departement')
      .select(`
        id,
    ...
    // 5. Formater les chefs AVEC les infos de rôle
    const chefsFormatted: ChefDistrict[] = (chefs || []).map((chef: any) => {
      const district = Array.isArray(chef.district) ? chef.district[0] : chef.district
      const fidele = Array.isArray(chef.fidele) ? chef.fidele[0] : chef.fidele
      const role = chef.role ? (Array.isArray(chef.role) ? chef.role[0] : chef.role) : null
      ...
      return {
        ...
        role_label: role?.label_role || '',
        role_couleur: '#9CA3AF' // couleur par défaut
      }
    })
  ...
  const toggleExpandDistrict = (districtId: number) => {
  ...
  const totalFideles = districtStats.reduce((sum, d) => sum + d.total_fideles, 0)
  const totalParoissesActives = districtStats.reduce((sum, d) => sum + d.paroisses.length, 0)
  ...
  return (
    <div className="min-h-screen ">
      <div className="max-w-7xl mx-auto ">
        ...
        {/* ============================================ */}
        {/* SECTION 1 : STATISTIQUES DES FIDÈLES */}
        {/* ============================================ */}
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            {statsLoading && <Loader2 size={16} className="animate-spin text-gray-400" />}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-white border border-gray-200 p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-wider">Total fidèles</p>
                  <p className="text-2xl font-light text-gray-900">{totalFideles}</p>
                </div>
              </div>
            </div>
            <div className="bg-white border border-gray-200 p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-wider">Paroisses avec membres</p>
                  <p className="text-2xl font-light text-gray-900">{totalParoissesActives}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {districtStats.length === 0 && !statsLoading ? (
              <div className="bg-white border border-gray-200 p-8 text-center">
                <Users size={32} className="text-gray-300 mx-auto mb-3" />
                <p className="text-gray-500">Aucun fidèle enregistré dans ce département</p>
              </div>
            ) : (
              districtStats.map(district => (
                <div key={district.district_id} className="bg-white border border-gray-200">
                  <button
                    onClick={() => toggleExpandDistrict(district.district_id)}
                    className="w-full px-5 py-4 flex items-center justify-between hover:bg-gray-50 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      {/* <MapPin size={16} className="text-gray-400" /> */}
                     District
                      <span className="font-medium text-gray-900">{district.district_nom}</span>
                      <span className="text-sm text-gray-500">
                        {district.total_fideles} fidèle{district.total_fideles > 1 ? 's' : ''}
                      </span>
                    ...
                    <ChevronDown 
                      ...
                      className={`text-gray-400 transition-transform ${
                        expandedDistricts.has(district.district_id) ? 'rotate-180' : ''
                      }`}
                    />
                  </button>
                  
                  {expandedDistricts.has(district.district_id) && (
                    <div className="border-t border-gray-100 px-5 py-3">
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                        {district.paroisses.map(paroisse => (
                          <div 
                            key={paroisse.paroisse_id}
                            className="flex items-center gap-2 p-2 bg-gray-50 border border-gray-100"
                          >
                            <span className="text-sm text-gray-700 truncate">{paroisse.paroisse_nom}</span>
                            <span className="ml-auto text-xs font-medium text-gray-500 bg-white px-2 py-0.5 border border-gray-200">
                              {paroisse.total_fideles}
                            </span>
                          </div>
                        ))}
                      </div>
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-gray-500 bg-gray-100 px-2 py-0.5">
                {chefsDistrict.filter(c => c.est_actif).length} actif{chefsDistrict.filter(c => c.est_actif).length > 1 ? 's' : ''}
              </span>
            </div>
            {chefsDistrict.length > 3 && (
              <button
                onClick={() => setShowAllChefs(!showAllChefs)}
                className="text-sm text-gray-500 hover:text-gray-700"
              >
                {showAllChefs ? 'Voir moins' : `Voir tout (${chefsDistrict.length})`}
              </button>
            )}
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {chefsDistrict.length === 0 ? (
              <div className="col-span-full bg-white border border-gray-200 p-6 text-center">
                <Shield size={24} className="text-gray-300 mx-auto mb-2" />
                <p className="text-gray-500 text-sm">Aucun chef de district nommé pour ce département</p>
              </div>
            ) : (
   
// Dans le map des chefs :
(showAllChefs ? chefsDistrict : chefsDistrict.slice(0, 3)).map(chef => {
  ...
  return (
    <div 
      ...
      className={`bg-white border p-4 ${
        chef.est_actif ? 'border-gray-200' : 'border-gray-100 opacity-60'
      }`}
    >
      <div className="flex items-start gap-3">
        <div className={`p-2 ${chef.est_actif ? 'bg-amber-50' : 'bg-gray-50'}`}>
          <UserCheck size={18} className={chef.est_actif ? 'text-amber-600' : 'text-gray-400'} />
        </div>
        <div className="min-w-0 flex-1">
          <p className="font-medium text-gray-900 truncate">
            {chef.fidele_prenom} {chef.fidele_nom}
          </p>
          
          {/* ✅ Badge du rôle (inspiré du composant admin) */}
          {chef.role_id ? (
            <span className={`inline-flex items-center gap-1.5 mt-1.5 text-xs px-2 py-0.5 border ${badgeColor}`}>
              <Shield size={12} />
              {roleLabel}
            </span>
          ) : (
          ...
          {!chef.est_actif && (
            <span className="inline-block mt-2 text-xs text-red-500 bg-red-50 px-2 py-0.5">
              Inactif
            </span>
          )}
        </div>
        ...
        {/* ============================================ */}
        {/* SECTION 3 : GESTION DES ANNÉES (déplacée en bas) */}
        {/* ============================================ */}
        <div className="border-t border-gray-200 pt-8">
          <div className="flex items-center gap-3 mb-4">
            <Calendar size={20} className="text-gray-700" />
            <h2 className="text-lg font-medium text-gray-900">
              Gestion des années de conférence
            </h2>
          </div>
          ...
          <div className="bg-gray-50 border border-gray-200 p-4 mt-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-gray-900 mb-1">Comment ça marche ?</p>
                <p className="text-sm text-gray-600">
                  <strong>Ouvrir</strong> : définit l'année de conférence en cours (<strong>{anneeEnCours?.annee?.label || 'N/A'}</strong>) comme année active pour les districts sélectionnés.
                  <br />
                  <strong>Fermer</strong> : désactive l'année en cours pour les districts sélectionnés.
                </p>
              </div>
            </div>
          </div>
