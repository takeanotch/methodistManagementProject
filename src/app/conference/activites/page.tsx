...
// app/conference/activites/page.tsx
'use client'
...
import { 
  getActivitesByUniteNiveau, 
  getActivitesStatsNiveau, 
  updateActiviteStatutNiveau, 
  deleteActiviteNiveau, 
  getActiviteFilesNiveau, 
  addFileToActiviteNiveau, 
  deleteActiviteFileNiveau, 
  updateActiviteCommentaireNiveau,
  getAnneesConferenceForUniteNiveau
} from '@/actions/activite-niveaux'
// ⚡ RÉUTILISATION des composants du district
import { ActiviteModalButtonNiveau } from '@/app/district/activites/ActiviteModalButtonNiveau'
...
export default function ConferenceActivitesPageWrapper() {
  return (
    <Suspense fallback={
      ...
      <ConferenceActivitesPage />
    </Suspense>
  )
}

function ConferenceActivitesPage() {
  ...
  const [currentAnneeId, setCurrentAnneeId] = useState<number | undefined>(anneeParam ? parseInt(anneeParam) : undefined)
  ...
  useEffect(() => {
    if (unite && currentAnneeId) {
      ...
      url.searchParams.set('annee_conference', currentAnneeId.toString())
  ...
  async function loadData() {
    try {
      ...
      // ⚡ Même logique que district
      let uniteData = await getDepartementUniteForConference(info.departement_id, info.conference_id)
      
      if (!uniteData) {
        const result = await ensureDepartementUniteExistsForConference(info.departement_id, info.conference_id)
      ...
      const annees = await getAnneesConferenceForUniteNiveau(uniteData.id, 'conference')
      setAnneesDisponibles(annees)

      let defaultAnneeId: number | undefined = currentAnneeId
      if (!defaultAnneeId) {
      ...
      if (defaultAnneeId && uniteData) {
        const activitesData = await getActivitesByUniteNiveau(uniteData.id, defaultAnneeId)
        const statsData = await getActivitesStatsNiveau(uniteData.id, defaultAnneeId)
  ...
  async function loadActivitesForAnnee() {
    if (unite && currentAnneeId) {
      const activitesData = await getActivitesByUniteNiveau(unite.id, currentAnneeId)
      const statsData = await getActivitesStatsNiveau(unite.id, currentAnneeId)
      setActivites(activitesData)
      setStats(statsData)
    }
  }
  ...
  async function openDetailsModal(activite: ActiviteWithDetails) {
    ...
    try {
      const files = await getActiviteFilesNiveau(activite.id)
  ...
  async function handleChangeStatut(activite: ActiviteWithDetails, statut: string) {
    ...
    const result = await updateActiviteStatutNiveau(activite.id, statut as any, 'conference')
  ...
  async function handleTerminateWithComment() {
    ...
    try {
      const commentResult = await updateActiviteCommentaireNiveau(activiteToTerminate.id, commentaireTerminer, 'conference')
      
      if (!commentResult.success) {
        console.error('Erreur mise à jour commentaire:', commentResult.error)
      }
      
      const result = await updateActiviteStatutNiveau(activiteToTerminate.id, 'termine', 'conference')
  ...
  async function handleDelete(activite: ActiviteWithDetails) {
    ...
    setActionLoading(activite.id)
    const result = await deleteActiviteNiveau(activite.id, 'conference')
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const result = await addFileToActiviteNiveau(formDataObj, 'conference')
    
    if (result.success) {
      const files = await getActiviteFilesNiveau(selectedActivite.id)
      setActiviteFiles(files)
      handleRefresh()
    } else {
  ...
  async function handleDeleteFile(fileId: number, fileUrl: string) {
    ...
    const result = await deleteActiviteFileNiveau(fileId, selectedActivite.id, fileUrl, 'conference')
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-4 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-light tracking-wide">Activités de la Conférence</h1>
      ...
      {!currentAnneeId ? (
        <div className="border border-gray-200 py-16 text-center bg-white">
          ...
          <p className="text-gray-400">Aucune année disponible pour cette conférence</p>
        </div>
                                      ...
                                      {canEdit && (
                                        <>
                                          ...
                                          {STATUTS.filter(s => s.value !== activite.statut).map(statut => {
                                            const IconComponent = statut.iconComponent
                                            return (
                                              <button
                                                key={statut.value}
      ...
      {/* Modal détails - ⚡ RÉUTILISATION du composant DetailsModal du district */}
      {selectedActivite && (
...
// ⚡ RÉUTILISATION de CalendrierView (copie exacte du district)
...
// ⚡ DetailsModal adapté pour Conférence
function DetailsModalConference({ 
  activite, 
  fichiers,
  onClose,
  onFileUpload,
...
// ⚡ RÉUTILISATION de TerminerAvecCommentaireModal
