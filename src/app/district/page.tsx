...
import { getPlansActionByDistrict } from '@/actions/plan-action-niveaux'
...
export default async function DistrictDashboardPage({ searchParams }: PageProps) {
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/district?id=eq.${chefInfo.district_id}&select=conference_id`, {
      headers: {
    ...
    conferenceId = district?.[0]?.conference_id
  } catch (error) {
    console.error('Erreur récupération conférence:', error)
  }
  ...
  const [
    ...
    getPlansActionByDistrict(chefInfo.district_id),
      ...
      <div className=" max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-4 mb-2">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mt-1">
                <h1 className="text-2xl font-light tracking-wide">District • {chefInfo.departement_nom}</h1>
                {unite && (
                  <ConfigButton 
                    uniteId={unite.id}
                    uniteNom={chefInfo.departement_nom}
                    uniteNiveau="District"
                  />
                )}
              </div>
        ...
        {anneesDisponibles && anneesDisponibles.length > 0 && (
          <div className="flex gap-2 mb-6">
            {anneesDisponibles.map((annee: any) => (
              <a
                key={annee.id}
                href={`/district?annee_conference=${annee.id}`}
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link
              href={buildUrl(`/district/projets`)}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <Link
                href={buildUrl(`/district/activites`)}
                className="text-xs text-gray-400 hover:text-black"
              >
                Voir tout →
              </Link>
            ...
            <div className="border border-gray-200 bg-white">
              {activitesRecentes.length === 0 ? (
                ...
                activitesRecentes.map((activite: any) => (
                  <Link
                    key={activite.id}
                    href={buildUrl(`/district/activites/${activite.id}`)}
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link
              href={buildUrl(`/district/plans-action`)}
              className="text-xs text-gray-400 hover:text-black"
            >
              Voir tout →
            </Link>
          ...
          <div className="border border-gray-200 bg-white">
            {plansFiltres.length === 0 ? (
              ...
              plansFiltres.slice(0, 5).map((plan: any) => (
                <Link
                  key={plan.id}
                  href={buildUrl(`/district/plans-action/${plan.id}`)}
                  className="block p-4 border-b border-gray-100 last:border-b-0 hover:bg-gray-50 transition-colors"
                >
