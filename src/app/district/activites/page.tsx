...
// app/district/activites/page.tsx
'use client'
...
import { 
  ...
  getPlansActionForUniteNiveau
} from '@/actions/activite-niveaux'
import { ActiviteModalButtonNiveau } from './ActiviteModalButtonNiveau'
...
export default function DistrictActivitesPage() {
  return (
    <Suspense fallback={<div>Chargement...</div>}>
      <DistrictActivitesContent />
    </Suspense>
  )
}

function DistrictActivitesContent() {
  ...
  async function loadData() {
    try {
      ...
      if (!uniteData) {
        const result = await ensureDepartementUniteExistsForDistrict(info.departement_id, info.district_id)
  ...
  async function handleChangeStatut(activite: ActiviteWithDetails, statut: string) {
    ...
    const result = await updateActiviteStatutNiveau(activite.id, statut as any, 'district')
  ...
  async function handleTerminateWithComment() {
    ...
    try {
      const commentResult = await updateActiviteCommentaireNiveau(activiteToTerminate.id, commentaireTerminer, 'district')
      
      if (!commentResult.success) {
        console.error('Erreur mise à jour commentaire:', commentResult.error)
      }
      
      const result = await updateActiviteStatutNiveau(activiteToTerminate.id, 'termine', 'district')
  ...
  async function handleDelete(activite: ActiviteWithDetails) {
    ...
    setActionLoading(activite.id)
    const result = await deleteActiviteNiveau(activite.id, 'district')
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const result = await addFileToActiviteNiveau(formDataObj, 'district')
  ...
  async function handleDeleteFile(fileId: number, fileUrl: string) {
    ...
    const result = await deleteActiviteFileNiveau(fileId, selectedActivite.id, fileUrl, 'district')
      ...
      <div className="p-6 max-w-7xl mx-auto">
        <div className="border border-gray-200 py-16 text-center bg-white">
          ...
          <p className="text-gray-500">
            L'unité d'organisation pour ce district n'a pas encore été créée.
          </p>
        </div>
      </div>
    )
                                ...
                                <div className="relative">
                                  ...
                                  {menuOpen === activite.id && (
                                    <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 shadow-lg z-10 min-w-[180px]">
                                      ...
                                      <Link
                                        href={`/district/activites/${activite.id}`}
...
function DetailsModal({ 
  ...
  const basePath = niveau === 'district' ? '/district' : '/conference'
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="p-4 border-t border-gray-200 flex gap-3">
          <Link
            href={`${basePath}/activites/${activite.id}`}
...
// Composant TerminerAvecCommentaireModal
