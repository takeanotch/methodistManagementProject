...
// app/paroisse/admin/departements/[id]/activites/page.tsx
'use client'
...
export default function ActivitesPage({ params, searchParams }: PageProps) {
  ...
  async function loadData() {
    try {
      ...
      const uniteData = await getDepartementUnite(departementId, fidele.paroisse_id)
      if (!uniteData) {
        setLoading(false)
        return
      }
      ...
      const annees = conferenceId ? await getAnneesConferenceByConference(conferenceId) : []
      ...
      if (defaultAnneeId && uniteData) {
        const activitesData = await getActivitesByUnite(uniteData.id, defaultAnneeId)
        const statsData = await getActivitesStats(undefined, uniteData.id, defaultAnneeId)
  ...
  async function handleChangeStatut(activite: ActiviteWithDetails, statut: string) {
    // Si on veut marquer comme terminé, ouvrir la modale de commentaire
    if (statut === 'termine') {
  ...
  async function handleTerminateWithComment() {
    ...
    try {
      ...
      // Mettre à jour l'activité sélectionnée si nécessaire
      ...
      <div className="mb-6">
        <div className="flex items-center gap-4 mb-2">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">{departement.nom}</p>
          </div>
        </div>
        <div className="flex gap-6 mb-6 border-b border-gray-200">
          ...
          <Link
            href={`/paroisse/admin/departements/${departementId}/plans-action`}
            className="px-1 py-3 text-sm text-gray-500 hover:text-black border-b-2 border-transparent hover:border-gray-300"
          >
            Plan d'action
          </Link>
                                ...
                                <div className="relative">
                                  ...
                                  {menuOpen === activite.id && (
                                    <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 shadow-lg z-10 min-w-[180px]">
                                      ...
                                      <Link
                                        href={`/paroisse/admin/departements/${departementId}/activites/${activite.id}`}
...
// Import React pour createElement
