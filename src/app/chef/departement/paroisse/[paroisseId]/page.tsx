// app/chef/departement/paroisse/[paroisseId]/page.tsx
...
import { getParoisseById } from '@/actions/chef-district'
import { getDepartementDataForParoisse, getAnneesDisponiblesForParoisse } from '@/actions/chef-district'
...
export default function ParoisseDetailPage({ params }: PageProps) {
  ...
  const [departementData, setDepartementData] = useState<any>(null)
  ...
  const departementId = departementIdParam ? parseInt(departementIdParam) : null
  ...
  }, [paroisseId, departementId])

  useEffect(() => {
    if (departementId && selectedAnnee !== null) {
      loadData()
    }
  }, [paroisseId, departementId, selectedAnnee])
 ...
 useEffect(() => {
    async function loadUserNiveau() {
      try {
        ...
        console.log('👤 Chef Département - Niveau utilisateur récupéré:', niveau)
        setUserNiveau(niveau)
      } catch (error) {
        console.error('Erreur récupération niveau:', error)
        // Valeur par défaut basée sur l'URL - chef de département = paroisse
        setUserNiveau('paroisse')
      }
    }
  ...
  async function loadConfiguration() {
    try {
      const config = await getConfiguration(parseInt(paroisseId))
      if (config) {
  ...
  async function loadBudgetData() {
    if (!uniteId || !selectedAnnee) {
      console.log('❌ Pas de uniteId ou selectedAnnee:', { uniteId, selectedAnnee })
      return
    }
    
    try {
      console.log('📊 Chargement budget pour uniteId:', uniteId, 'annee:', selectedAnnee)
      ...
      console.log('✅ Totals réalisés:', totalsData)
  ...
  async function loadAnnees() {
    try {
      ...
      if (anneeId && anneeId.toString() !== anneeParam) {
        router.replace(
          `/chef/departement/paroisse/${paroisseId}?departementId=${departementId}&annee=${anneeId}`
        )
      }
  ...
  async function loadData() {
    try {
      ...
      const paroisseData = await getParoisseById(parseInt(paroisseId))
      setParoisse(paroisseData)
      
      if (departementId && selectedAnnee) {
        // Récupérer l'uniteId du département dans cette paroisse
        const unite = await getDepartementUnite(departementId, parseInt(paroisseId))
        
        if (unite) {
          console.log('✅ UniteId trouvé:', unite.id)
          setUniteId(unite.id)
        } else {
          console.warn('⚠️ Aucune unité trouvée pour departementId:', departementId, 'paroisseId:', paroisseId)
        }
  ...
  async function loadProjetFichiers(projetId: number) {
    ...
    try {
      ...
      setProjetFichiers(prev => ({ ...prev, [projetId]: [] }))
    } finally {
  ...
  function getFileIcon(typeFichier: string) {
    ...
    } else if (typeFichier.includes('spreadsheet') || typeFichier.includes('excel') || typeFichier.includes('csv')) {
      return <FileSpreadsheet size={16} className="text-green-500" />
    } else if (typeFichier.includes('document') || typeFichier.includes('word')) {
      return <FileText size={16} className="text-blue-500" />
    } else if (typeFichier.includes('zip') || typeFichier.includes('rar') || typeFichier.includes('tar')) {
  ...
  const filteredActivites = (departementData?.activites || []).filter((activite: any) => {
    ...
    return (
      <div className="p-6 max-w-7xl mx-auto">
        <div className="text-center py-12">
          ...
          <h2 className="text-lg font-medium mb-2">Paramètre manquant</h2>
          <p className="text-gray-500">L'identifiant du département est requis</p>
          <Link href="/chef/departement/district" className="mt-4 inline-block text-sm text-gray-500 hover:text-black">
            Retour à la liste
          </Link>
        </div>
      </div>
      ...
      {!selectedAnnee && anneesDisponibles.length > 0 ? (
        ...
        <>
          ...
          <div className="grid grid-cols-5 gap-3 mb-8">
            <div className="bg-white border border-gray-200 p-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-2xl font-light">{departementData.totalFideles}</span>
              </div>
            ...
            <div className="bg-green-50 border border-green-200 p-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-2xl font-light text-green-700">{departementData.actifs}</span>
              </div>
            ...
            <div className="bg-gray-50 border border-gray-200 p-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-2xl font-light text-gray-500">{departementData.inactifs}</span>
              </div>
            ...
            <div className="bg-white border border-gray-200 p-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-2xl font-light">{departementData.activites?.length || 0}</span>
              </div>
            ...
            <div className="bg-white border border-gray-200 p-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-2xl font-light">{departementData.projets?.length || 0}</span>
              </div>
          ...
          <div className="flex gap-6 mb-6 border-b border-gray-200 overflow-x-auto">
            ...
            >
              Membres ({departementData.totalFideles})
            </button>
            <button
            ...
            >
              Activités ({departementData.activites?.length || 0})
            </button>
            <button
            ...
            >
              Plans d'action ({departementData.plansAction?.length || 0})
            </button>
            <button
            ...
            >
              Projets ({departementData.projets?.length || 0})
            </button>
            <button
          ...
          <div>
            ...
            {/* {activeTab === 'membres' && (
              <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    {departementData.fideles?.length === 0 ? (
                      <tr>
                      ...
                      departementData.fideles.map((affectation: any) => {
                        ...
                        return (
                          <tr key={affectation.id} className="border-b border-gray-100 hover:bg-gray-50">
                            ...
                            <td className="py-3 px-4 text-sm">
                              {affectation.role_details?.nom_role || affectation.role_details?.label_role || '-'}
                            </td>
            ...
            {activeTab === 'membres' && (
              <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    {!departementData?.fideles || departementData.fideles.length === 0 ? (
                      <tr>
                      ...
                      departementData.fideles.map((affectation: any) => {
                        ...
                        return (
                          <tr key={affectation.id} className="border-b border-gray-100 hover:bg-gray-50">
                            {/* Nouvelle colonne : Image de profil */}
                            <td className="py-3 px-4">
                            ...
                            <td className="py-3 px-4">
                              <div>
                                <Link 
                                  href={`/paroisse/departements/fideles/${fidele?.id}`}
            ...
            {/* TAB ACTIVITES - Version lecture seule avec vue liste/calendrier */}
            {activeTab === 'activites' && (
              <div>
                {/* Barre d'outils */}
            ...
            {activeTab === 'plans' && (
              <div className="space-y-3">
                {!departementData.plansAction || departementData.plansAction.length === 0 ? (
                  <div className="text-center py-12 bg-white border border-gray-200">
                  ...
                  departementData.plansAction.map((plan: any) => (
                    <div key={plan.id} className="bg-white border border-gray-200 p-4">
                      ...
                      <div className="flex items-center gap-4 mt-3 text-xs text-gray-400">
                        ...
                        <span className={`px-2 py-1 ${
                          plan.statut === 'en_cours' ? 'bg-yellow-50 text-yellow-700' :
                          plan.statut === 'termine' ? 'bg-green-50 text-green-700' : 'bg-gray-50 text-gray-600'
                        }`}>
            ...
            {activeTab === 'projets' && (
              <div className="space-y-4">
                {!departementData.projets || departementData.projets.length === 0 ? (
                  <div className="text-center py-12 bg-white border border-gray-200">
                  ...
                  <>
                    <div className="grid grid-cols-3 gap-4 mb-6">
                      <div className="bg-blue-50 border border-blue-200 p-4 text-center">
                        <div className="text-2xl font-light text-blue-700">{departementData.projetsStats?.total || 0}</div>
                        <div className="text-xs text-blue-600">Total projets</div>
                      </div>
                      <div className="bg-yellow-50 border border-yellow-200 p-4 text-center">
                        <div className="text-2xl font-light text-yellow-700">{departementData.projetsStats?.enCours || 0}</div>
                        <div className="text-xs text-yellow-600">En cours</div>
                      </div>
                      <div className="bg-green-50 border border-green-200 p-4 text-center">
                        <div className="text-2xl font-light text-green-700">{departementData.projetsStats?.termines || 0}</div>
                        <div className="text-xs text-green-600">Terminés</div>
                      </div>
                    </div>
                    
                    {departementData.projets.map((projet: any) => {
                            ...
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                ...
                                <div className="flex flex-wrap items-center gap-4 mt-3 text-xs">
                                  ...
                                  {projet.date_fin && (
                                    <span className="text-gray-500">
                                      Fin prévue: {new Date(projet.date_fin).toLocaleDateString('fr-FR')}
                                    </span>
                                  )}
                                  <span className={`px-2 py-1 ${
                                ...
                                {projet.plan_action && (
                                  <p className="text-xs text-gray-400 mt-2">
                                    Plan d'action lié: {projet.plan_action.titre}
                                  </p>
                                )}
                                {projet.budget && (
                                  <p className="text-xs text-gray-400 mt-1">
                                    Budget: {new Intl.NumberFormat('fr-FR').format(projet.budget.montant)} {projet.budget.currency}
                                  </p>
                                )}
                              </div>
                              ...
                              <div className="flex items-center gap-2">
                                <button
                                  onClick={() => toggleProjetExpanded(projet.id)}
                                  className="flex items-center gap-1 px-3 py-1 text-xs border border-gray-300 hover:border-black hover:bg-gray-50 transition-colors"
                                >
                                  <Paperclip size={12} />
                                ...
                                <button
                                  onClick={() => openProjetDetailsModal(projet)}
                                  className="p-1 text-gray-400 hover:text-black"
                              ...
                              {isLoadingFichiers ? (
                                ...
                                <div className="space-y-2">
                                  {fichiers.map((fichier: any) => (
                                    <div 
                                      key={fichier.id} 
                                      className="flex items-center justify-between bg-white border border-gray-200 p-3 hover:border-gray-300"
                                    >
                                      <div className="flex items-center gap-3">
                                      ...
                                      <button
                                        onClick={() => handleDownloadFichier(fichier.id, fichier.nom_fichier)}
                                        disabled={downloadingFile === fichier.id}
                                        className="p-2 text-gray-500 hover:text-black hover:bg-gray-100 transition-colors"
                                        title="Télécharger"
                                      >
                                        {downloadingFile === fichier.id ? (
    ...
    {uniteId && selectedAnnee ? (
      ...
      {showProjetDetailsModal && selectedProjetDetails && (
        <ProjetDetailsModal
          ...
          onClose={() => {
            setShowProjetDetailsModal(false)
...
// ... (le reste des composants reste inchangé : BudgetRow, CalendrierView, ActiviteDetailsModal, ProjetDetailsModal, BudgetDetailsModal)
...
function ProjetDetailsModal({ 
  ...
  projet: any
  fichiers: any[]
  loadingFichiers: boolean
  onClose: () => void
  onDownload: (fichierId: number, nomFichier: string) => Promise<void>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div>
              ...
              <span className="px-2 py-1 bg-gray-100 text-gray-600 text-sm">
                {typeLabels[projet.type] || projet.type}
              </span>
            </div>
            <div>
          ...
          {projet.plan_action && (
            <div className="mb-6">
              <div className="text-xs text-gray-500 mb-1">Plan d'action lié</div>
              <div className="text-sm">{projet.plan_action.titre}</div>
            </div>
          )}

          {projet.budget && (
            <div className="mb-6">
              <div className="text-xs text-gray-500 mb-1">Budget alloué</div>
              <div className="text-sm">
                {new Intl.NumberFormat('fr-FR').format(projet.budget.montant)} {projet.budget.currency}
                <span className="text-xs text-gray-400 ml-2">({projet.budget.libelle})</span>
              </div>
            </div>
          )}
            ...
            {loadingFichiers ? (
              ...
              <div className="space-y-2">
                {fichiers.map((fichier: any) => (
                  <div 
                    key={fichier.id} 
                    className="flex items-center justify-between bg-gray-50 border border-gray-200 p-3 hover:border-gray-300"
                  >
                    <div className="flex items-center gap-3">
                      {getFileIcon(fichier.type_fichier)}
                      <div>
                        ...
                        <p className="text-xs text-gray-400">
                          {formatFileSize(fichier.taille_fichier)} • 
                          Ajouté le {new Date(fichier.uploaded_at).toLocaleDateString('fr-FR')}
                        </p>
                      </div>
                    </div>
                    
                    <button
                      onClick={() => onDownload(fichier.id, fichier.nom_fichier)}
                      disabled={downloadingFile === fichier.id}
                      className="p-2 text-gray-500 hover:text-black hover:bg-gray-200 transition-colors"
                      title="Télécharger"
                    >
                      {downloadingFile === fichier.id ? (
                        <Loader2 size={16} className="animate-spin" />
